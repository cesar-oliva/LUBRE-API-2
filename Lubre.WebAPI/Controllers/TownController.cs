...
namespace Lubre.WebAPI.Controllers;
    ...
    public class TownController : Controller
    {
        ...
        public async Task<IActionResult> GetOne(Guid id)
        {
            ...
            try
            {
                var unit = await _townRepository.GetByIdAsync(id);
        ...
        [HttpPost]
        public async Task<IActionResult> Save(RegisterTownRequestDTO townDto) 
        {
            try
            {
                if(ModelState.IsValid){
                var newTown = await _townRepository.AddAsync(townDto);
                return CreatedAtAction("GetOne", new { id = newTown.Id }, newTown);
                }
                return new JsonResult("Something went wrong") { StatusCode = 500 };
            }
            catch (System.Exception)
        ...
        [HttpPut]
        [Route("{id}")]
        public async Task<IActionResult> Update(Guid id, RegisterTownRequestDTO dto)
        {
            if(id.Equals(Guid.Empty)) return new JsonResult("Not Found") { StatusCode = 400 };
            if (dto == null) return new JsonResult("Not Found") { StatusCode = 400 };
            try
            {    
                var unit = await _townRepository.UpdateAsync(id,dto);
                return new JsonResult(unit) { StatusCode = 200 };      
            }
            catch (System.Exception)
            {
        ...
        public IActionResult Delete(Guid id)
        {
            ...
            try
            {
                _townRepository.DeleteAsync(id);
